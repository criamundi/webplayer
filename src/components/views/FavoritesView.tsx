...
function FavoriteCover({ item }: { item: FavoriteItem }) {
  ...
  const isMedia = item.id.startsWith('movie:') || item.id.startsWith('series:');
  ...
  return source && !failed
    ? <img
        ...
        onError={() => {
          const proxy = getPlayableStreamUrl(item.logo || '');
          if (proxy && source !== proxy) setSource(proxy);
          else setFailed(true);
        }}
        className={`h-full w-full ${isMedia ? 'object-cover' : 'object-contain p-8'}`}
      />
