...
export function ProviderAdminsView() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Loader2 className="mx-auto animate-spin text-white/40" /> : admins.length === 0 ? <p className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-sm text-white/40">Nenhum administrador de provedor criado.</p> : <div className="space-y-2.5">{admins.map((admin) => <AdminRow key={admin.id} admin={admin} providers={providers} busy={busyId === admin.id} onSave={(provider, displayName) => action(admin.id, { action: 'update', providerId: provider, displayName }, 'Administrador atualizado.')} onToggle={() => action(admin.id, { action: 'toggle' }, admin.admin_active ? 'Acesso desativado.' : 'Acesso ativado.')} onPassword={() => resetPassword(admin)} onDelete={() => remove(admin)} />)}</div>}
  </div>;
}

function AdminRow({ admin, providers, busy, onSave, onToggle, onPassword, onDelete }: { admin: Profile; providers: Provider[]; busy: boolean; onSave: (providerId: string, name: string) => void; onToggle: () => void; onPassword: () => void; onDelete: () => void; }) {
  const [provider, setProvider] = useState(admin.provider_id || ''), [name, setName] = useState(admin.display_name || '');
  return <div className={`rounded-2xl border p-4 ${admin.admin_active ? 'border-white/10 bg-white/[.03]' : 'border-red-300/10 bg-red-300/[.03] opacity-70'}`}><div className="flex flex-wrap items-center gap-3"><span className={`flex h-10 w-10 items-center justify-center rounded-xl ${admin.admin_active ? 'bg-lime-300/15 text-lime-300' : 'bg-white/5 text-white/30'}`}><ShieldCheck className="h-5 w-5" /></span><div className="min-w-[180px] flex-1"><p className="text-sm font-semibold">{admin.email}</p><p className="text-xs text-white/40">{admin.admin_active ? 'Acesso ativo' : 'Acesso desativado'}</p></div><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome" className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs" /><select value={provider} onChange={(e) => setProvider(e.target.value)} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs">{providers.map((p) => <option className="bg-slate-900" value={p.id} key={p.id}>{p.name}</option>)}</select><div className="flex gap-1">{busy ? <Loader2 className="m-2 h-4 w-4 animate-spin" /> : <><button title="Salvar nome e provedor" onClick={() => onSave(provider, name)} className="rounded-lg p-2 text-white/50 hover:bg-white/10 hover:text-white"><Save className="h-4 w-4" /></button><button title="Trocar senha" onClick={onPassword} className="rounded-lg p-2 text-sky-300 hover:bg-sky-300/10"><KeyRound className="h-4 w-4" /></button><button title={admin.admin_active ? 'Desativar acesso' : 'Ativar acesso'} onClick={onToggle} className={`rounded-lg p-2 ${admin.admin_active ? 'text-amber-300' : 'text-emerald-300'}`}><Power className="h-4 w-4" /></button><button title="Excluir" onClick={onDelete} className="rounded-lg p-2 text-red-300 hover:bg-red-300/10"><Trash2 className="h-4 w-4" /></button></>}</div></div></div>;
}
