...
interface Branding { provider_id?: string; app_name: string; logo_url: string | null; background_url: string | null; login_background_url: string | null; primary_color: string; secondary_color: string; player_layout: string; settings_layout: string; main_font_scale: number; home_feature_type: 'latest_movie' | 'latest_series'; }
const empty = (providerId?: string): Branding => ({ ...(providerId ? { provider_id: providerId } : {}), app_name: 'Nexus Play', logo_url: null, background_url: null, login_background_url: null, primary_color: '#bef264', secondary_color: '#091018', player_layout: 'default', settings_layout: 'default', main_font_scale: 1.3, home_feature_type: 'latest_movie' });
...
export function BrandingView() {
  ...
  useEffect(() => { if (!target) return; void (async () => {
    ...
    const query = target === 'global'
      ? supabase.from('app_branding').select('app_name, logo_url, background_url, login_background_url, primary_color, secondary_color, player_layout, settings_layout, main_font_scale, home_feature_type').eq('singleton', true).maybeSingle()
  ...
  const save = async (event: FormEvent) => {
    ...
    const payload = { app_name: branding.app_name.trim() || 'Nexus Play', logo_url: branding.logo_url, background_url: null, login_background_url: branding.login_background_url, primary_color: branding.primary_color.toUpperCase(), secondary_color: '#091018', player_layout: branding.player_layout, settings_layout: branding.settings_layout, main_font_scale: Math.min(1.3, Math.max(.85, Number(branding.main_font_scale) || 1)), home_feature_type: branding.home_feature_type };
