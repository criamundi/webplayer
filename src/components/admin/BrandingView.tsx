...
export function BrandingView() {
  const [providers, setProviders] = useState<Provider[]>([]), [target, setTarget] = useState('');
  ...
  useEffect(() => { void (async () => {
    ...
    const { data: profile } = await supabase.from('profiles').select('provider_id, role').eq('id', auth.user.id).maybeSingle();
    const superRole = profile?.role === 'super_admin' || profile?.role === 'admin';
    setIsSuper(superRole);
    if (superRole) {
      const { data } = await supabase.from('iptv_providers').select('id, name').order('name');
      setProviders(data || []); setTarget('global');
    } else if (profile?.provider_id) {
      const { data } = await supabase.from('iptv_providers').select('id, name').eq('id', profile.provider_id).maybeSingle();
      setProviders(data ? [data] : []); setTarget(profile.provider_id);
  ...
  const save = async (event: FormEvent) => {
    ...
    const payload = { app_name: branding.app_name.trim() || 'Nexus Play', logo_url: branding.logo_url, background_url: branding.background_url, login_background_url: branding.login_background_url, primary_color: branding.primary_color, secondary_color: branding.secondary_color, player_layout: branding.player_layout, settings_layout: branding.settings_layout };
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-semibold">Branding</h1><p className="mt-2 text-sm text-white/45">{target === 'global' ? 'Identidade do painel Super Admin e padrão inicial do app.' : 'Identidade exclusiva deste provedor.'}</p></div>{isSuper && <label><span className="mb-2 block text-xs text-white/50">Editar identidade</span><select value={target} onChange={(e) => setTarget(e.target.value)} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm"><option className="bg-slate-900" value="global">Painel Super Admin</option>{providers.map((provider) => <option className="bg-slate-900" key={provider.id} value={provider.id}>{provider.name}</option>)}</select></label>}</div>
