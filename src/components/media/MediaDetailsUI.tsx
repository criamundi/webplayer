...
export function MediaArrowRow({ title, children }: { title: string; children: ReactNode }) {
  ...
  return <div>
    ...
    <div ref={trackRef} data-tv-axis="horizontal" className="media-arrow-track media-poster-group flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth px-2 py-3 scrollbar-none">{children}</div>
  </div>;
}
