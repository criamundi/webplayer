...
export function mediaDuration(value: unknown): string {
  const duration = mediaText(value).trim();
  if (!duration || /^(?:n\/?a|null|undefined|indispon[ií]vel)$/i.test(duration)) return '';

  const numericParts = duration.replace(',', '.').match(/\d+(?:\.\d+)?/g);
  if (numericParts?.length && numericParts.every((part) => Number(part) === 0)) return '';
